...
class IP(Structure):
  ...
  # on 64 bit Python version "c_ulong" is 8 bit length then fields "src" and "dst" have to be set to "c_uint" or another 4 bit length type
  _fields_ = [
    ("ihl",          c_ubyte, 4),
    ("version",      c_ubyte, 4),
    ("tos",          c_ubyte),
...
try:
  while True:
    ...
    if IPAddress(ip_header.src_address) in IPNetwork(subnet) and \
       ip_header.src_address != host:
...
# handle CTRL-C
  ...
  # if we're using Windows, turn off primiscuous mode
  if os.name == "nt":
